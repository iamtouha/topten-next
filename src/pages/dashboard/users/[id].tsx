...
const User: NextPage = () => {
  ...
  const { data: user, status, error } = trpc.user.findUserById.useQuery({ id });
  const editRole = trpc.user.editRole.useMutation({
    onSuccess: async (todo) => {
      await utils.user.getAllUsers.cancel();
      console.log(todo);
    },
  });
  ...
  if (status === "error") {
    return (
      <div className="grid min-h-screen place-items-center text-base container-res md:text-lg">
        {error.message} | {error.data?.httpStatus ?? 500}
      </div>
    );
  }
